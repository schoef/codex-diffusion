...
\end{align}
For the Jacobi bridge used in \eqref{eq:qvf-coefficient-transport}, this same parameter choice gives
\[
b_n^{(\eta)}=-(n+r)\cot\phi,
\qquad
\phi=\frac{\pi}{2}+\frac{\eta}{2},
\]
so in particular $b_0^{(\eta)}=-r\cot\phi=r\tan(\eta/2)=\eta^\ast(\eta)=\mu$, as required by the centering identity.
